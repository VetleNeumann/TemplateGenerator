...
namespace Project
{
	public partial class PositionSystem : ISystem<Position, Position.Vectorized, Position.Array>
	{
		...
		public void Update<T1Arch, T2Arch>(ref ComponentEnumerableNew<Position, Position.Vectorized, Position.Array>.Enumerator<T1Arch, T2Arch> en)
			where T1Arch : unmanaged, IArchType<T1Arch, Position, Position.Vectorized, Position.Array>
			where T2Arch : unmanaged, IArchType<T2Arch, Position, Position.Vectorized, Position.Array>
		{
			while (en.MoveNext())
			{
				var item = en.Current;
